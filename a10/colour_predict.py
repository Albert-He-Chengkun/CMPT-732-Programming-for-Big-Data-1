...
def main(inputs):
    ...
    classifier = MultilayerPerceptronClassifier(featuresCol='features', labelCol='label', layers=[3, 30, 11], seed=2023)
    ...
    lab_indexer = StringIndexer(inputCol='word', outputCol='label')
    lab_classifier = MultilayerPerceptronClassifier(featuresCol='features', labelCol='label', layers=[3, 30, 11], seed=2023)
    lab_pipeline = Pipeline(stages=[sql_transformer, lab_assembler, lab_indexer, lab_classifier])
