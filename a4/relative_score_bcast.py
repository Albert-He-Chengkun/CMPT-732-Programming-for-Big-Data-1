...
def relative_score_calculator_bcast(commentbysub, bcast):
    '''
    calculate relative score with broadcasted data
    returns: key value pair of relative score & author in tuple
    '''
    subreddit = commentbysub[0]
    score = commentbysub[1][0]
    author = commentbysub[1][1]
    average_score = bcast.value[subreddit]
...
def main(inputs, output):
    ...
    average_data_bacst = sc.broadcast(dict(filtered_average_data.collect()))

    commentbysub = commentdata.map(lambda c: (c['subreddit'], (c['score'],c['author'])))
    final_data = commentbysub.map(partial(relative_score_calculator_bcast, bcast=average_data_bacst)).sortBy(lambda x: x[0], ascending=False)
